...
const initialState: State = {
  ...
  tokenId: null,
  userName: "",
  minimumCakeRequired: new BigNumber(totalCost).multipliedBy(
...
const ProfileCreationProvider: React.FC = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const canMint = await bunnyFactoryContract.methods
        .canMint(account)
        .call();
  ...
  const actions: ContextType["actions"] = useMemo(
    () => ({
      ...
      setTokenId: (tokenId: number) =>
        dispatch({ type: "set_tokenid", tokenId }),
  ...
  return (
    <ProfileCreationContext.Provider value={{ ...state, actions }}>
      {children}
