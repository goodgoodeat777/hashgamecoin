...
describe("Config IFOs", () => {
  ...
  it.each(ifos.map((ifo) => [ifo.id, ifo.address]))(
    "IFO #%d has an unique address",
    (id, address) => {
      const duplicates = ifos.filter((i) => address === i.address);
      expect(duplicates).toHaveLength(1);
    }
  );
  it.each(ifos.map((ifo) => [ifo.id, ifo.campaignId]))(
    "IFO #%d has an unique campaign ID",
    (id, campaignId) => {
  ...
  it.each(ifos.map((ifo) => [ifo.id, ifo.campaignId]))(
    "IFO #%d has an existing campaign ID",
    (id, campaignId) => {
